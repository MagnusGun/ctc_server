...
async fn get_room_temp(State((tx, _)): State<(ModbusSender, TemperatureValidationConfig)>) -> Result<String, ApiError>  {
    read_parameter(&tx, CTC_ROOM_TEMP, "room_temperature", "get_room_temp").await
}

// read the room temperature setpoint
async fn get_room_set_temp(State((tx, _)): State<(ModbusSender, TemperatureValidationConfig)>) -> Result<String, ApiError> {
...
async fn set_room_set_temp(State((tx, config)): State<(ModbusSender, TemperatureValidationConfig)>, Query(param): Query<RoomSetPoint>) -> Result<String, ApiError>{
...
async fn get_outdoor_temp(State((tx, _)): State<(ModbusSender, TemperatureValidationConfig)>) -> Result<String, ApiError> {
    read_parameter(&tx, CTC_OUTDOOR_TEMP, "outdoor_temperature", "get_outdoor_temp").await
}

// read heatpump outgoing flow temperature
async fn get_flow_temp(State((tx, _)): State<(ModbusSender, TemperatureValidationConfig)>) -> Result<String, ApiError> {
...
async fn get_return_temp(State((tx, _)): State<(ModbusSender, TemperatureValidationConfig)>) -> Result<String, ApiError> {
