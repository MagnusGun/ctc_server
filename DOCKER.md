...
| `CTC_HOMEY_ENABLED` | `false` | Enable Homey REST integration (controls Cirkulationspump) |
| `CTC_HOMEY_URL` | (empty) | Homey Pro LAN URL, e.g. `http://192.168.10.10` |
| `CTC_HOMEY_TOKEN` | (empty) | Homey PAT (scopes: `homey.device.control`, `homey.device.readonly`) |
| `CTC_HOMEY_PUMP_DEVICE_ID` | (empty) | Device id of the smart plug acting as pump switch |
| `CTC_HOMEY_POLL_INTERVAL_SECS` | `60` | Drift reconciliation interval; `0` = push-only |
